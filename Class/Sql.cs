...
namespace APIOffice
{
    public abstract class SQLHelper
    {
        ...
        /// </summary>
        private static readonly string connectionString = ConfigurationHelper.Instance.GetConnectionString("DefaultConnection", "");
        ...
        /// </summary>
        /// <param name="cmdText">存储过程的名字或者 T-SQL 语句</param>
        /// <param name="commandParameters">以数组形式提供SqlCommand命令中用到的参数列表1</param>
        /// <returns>返回一个表集合(DataTableCollection)表示查询得到的数据集</returns>
        ...
        /// <summary>
        /// add parameter array to the cache将参数数组添加到缓存中
        /// </summary>
        /// <param name="cacheKey">Key to the parameter cache 参数缓存的键</param>
        /// <param name="cmdParms">an array of SqlParamters to be cached 被缓存的参数数组</param>
        ...
        /// <summary>
        /// Retrieve cached parameters 检索缓存参数
        /// </summary>
        /// <param name="cacheKey">key used to lookup parameters 用于查找参数的键 </param>
        /// <returns>Cached SqlParamters array 缓存参数数组</returns>
        ...
        public static void ConnectionOpen()
        {
            try
            {
                ...
                //string constr = "Data Source=wjf1;Initial Catalog=erp;User ID=sa;password=";
                //string constr = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=E:\\myobject\\Ant\\AntOA\\db\\lmtof.mdb";
                ...
                if (Conn1 == null)
                {
                    Conn1 = new DbConnection(constr);
                    Conn1.Open();
                }
                else if (Conn1.State == System.Data.ConnectionState.Closed)
                {
                    Conn1.ConnectionString = constr;
                    Conn1.Open();
                }
                else if (Conn1.State == System.Data.ConnectionState.Broken)
                {
                    Conn1.Close();
                    Conn1.ConnectionString = constr;
                    Conn1.Open();
        ...
        public static void ConnectionClose()
        {
            try
            {
                if (Conn1.State.Equals(ConnectionState.Open))
                {
                    //关闭连接
                    Conn1.Close();
                    //释放占用的资源
        ...
        public static DataTable GetDataTable1(string strSql)
        {
            ...
            {
                ConnectionOpen();
                DbConnection cn1 = Conn1;

                DbCommand cmd = new DbCommand(strSql, cn1);
            ...
            catch (Exception)
            {
                ...
                //WriteErrLog(e.Message, "ExecuteCommand", "");
            }
            return dt;

        }
        public static string ExecuteCommand1(string strSql)
        {
            return ExecuteCommand1(strSql, new SqlParameter[0]);
        }
        /// <summary>Command类 执行SQL语句
        /// </summary>
        /// <param name="SQL">SQL语句</param>
        /// <param name="values">存储过程参数集，或NULL</param>
        /// <returns>影响记录数</returns>
        public static string ExecuteCommand1(string strSql, params DbParameter[] values)
        {
            ...
            try
            {
                ...
                DbCommand cmd = new DbCommand(strSql, cn1);
                //cmd.CommandType = CommandType.StoredProcedure;      //cmd类型为存储过程类型
                if (values != null)
